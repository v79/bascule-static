...
/**
 * Generates paginated post navigation pages for all layouts defined in project.config.postLayouts, or just "post"s
 *
...
class PostNavigationGenerator(posts: List<Post>, numPosts: Int = 1, postsPerPage: Int) : GeneratorPipeline,
    ...
    override suspend fun process(
        ...
        if (project.config.postLayouts.isEmpty()) {
            project.config.postLayouts = setOf("post")
        }

        project.config.postLayouts.forEach { layout ->
            ...
            val postsFolder = fileHandler.createDirectory(project.config.directories.output.absolutePath, "${layout}s")
